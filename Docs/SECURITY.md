...
- SCRAM доступен в вариантах SHA-512, SHA-256 и SHA-1 (SHA-512 —
  реализация Luma), а также в PLUS-вариантах с канальной привязкой
  (SCRAM-SHA-512-PLUS > SHA-256-PLUS > SHA-1-PLUS): gs2-заголовок
  «p=tls-exporter,» (RFC 9266) или «p=tls-server-end-point,» доказывает,
  что SASL-обмен идёт по тому же TLS-каналу, который видит клиент, и
  ломает MITM-ретрансляцию. PLUS выбирается только когда сервер
  рекламирует соответствующий channel-binding тип, иначе — обычный SCRAM;
  механизм выбирается по рекламе сервера.
- OMEMO работает в legacy-неймспейсе `eu.siacs.conversations.axolotl` (так
  умеет закреплённая версия MartinOMEMO) и в OMEMO 2 (`urn:xmpp:omemo:2`,
  payload AES-256-CBC + HMAC-SHA-256 в конверте SCE `urn:xmpp:sce:0`). Luma
  публикует omemo:2 bundle и список устройств, делит Double Ratchet-сессии с
  legacy-модулем и помечает протокол каждого ключа на экране шифрования.
- Канал шифруется STARTTLS/direct TLS собственным TLS-процессором Luma на
  OpenSSL 3 (SecureTransport ограничивает потоковый TLS версией 1.2 и не
  имеет keying exporter): минимум TLS 1.2, максимум TLS 1.3, ALPN
  «xmpp-client», SNI по домену XMPP. Сертификат проверяется через
  SecTrust с серверной политикой домена из JID (та же логика
  CertificateTrustEvaluator). Фактическая версия TLS, шифр и выбранный
  SASL-механизм (самый стойкий из рекламируемых сервером, с канальной
  привязкой при её доступности) показываются на экране информации о
  сервере.
...
- Архивные эхо собственных служебных станз (receipts, chat markers,
  повторно доставленные копии) и собственные сообщения, которые невозможно
  расшифровать обратно (нет encrypt-to-self ключа или self-сессия устарела),
  не превращаются в пузыри «Не удалось расшифровать сообщение»: они
  отбрасываются, как это делают другие клиенты, чтобы не засорять историю
  фантомами, которых нет в виде сообщений на сервере.
- Сохранение медиа выполняется только по явному действию пользователя:
  фото/видео/видеосообщения — в медиатеку, файлы и голосовые — в «Файлы»
  (Загрузки). Расшифрованное содержимое при этом покидает sandbox приложения.
